...
# 
# 
# # give all games number 1 to start
# bh_all.by.game$game <- 1
# 
# # modify for rows that need game number >1 based on bh_all.by.game.trim
# bh_all.by.game$game[3] <- 2
# bh_all.by.game$game[6] <- 2
# bh_all.by.game$game[8] <- 2
# bh_all.by.game$game[9] <- 2
# bh_all.by.game$game[10] <- 2
# bh_all.by.game$game[12] <- 3
# bh_all.by.game$game[13] <- 3
# 
# 
# # check out those columns
# bh_all.by.game[, c("date", "opponent", "game")]
# 
# 
# 
# # now modify our dataframe with game numbers
# # replace values in games column conditional upon values in opponent and dates column
# 
# # set all games to 1 to start off
# bh_all$game <- 1
# 
# # replace values in games column conditional upon values in opponent and dates column
# # if order by date, are going to have to redo these
# bh_all$game[bh_all$opponent == 'Alleycats' & bh_all$date == '2015-05-19'] <- 2
# bh_all$game[bh_all$opponent == 'Thunderbirds' & bh_all$date == '2015-06-28'] <- 2
# bh_all$game[bh_all$opponent == 'Revolution' & bh_all$date == '2015-05-07'] <- 2
# bh_all$game[bh_all$opponent == 'Windchill' & bh_all$date == '2015-05-20'] <- 2
# bh_all$game[bh_all$opponent == 'Radicals' & bh_all$date == '2015-06-20'] <- 2
# bh_all$game[bh_all$opponent == 'Windchill' & bh_all$date == '2015-04-29'] <- 3
# bh_all$game[bh_all$opponent == 'Radicals' & bh_all$date == '2015-05-20'] <- 3
...
# # assign a column for unique game
# # _______ now doing this with data.table _______
# bh_all <- bh_all %>%
#   group_by(opponent, game) %>% 
#   nest() %>%            
#   mutate(ind.game = seq(1, nrow(bh_all.by.game))) %>%
#   unnest()
# bh_all
